...
void step12() {
  ...
  //stroke(0, 255, 0);
  //strokeWeight(3);
  line(saveIntersectionX[0], saveIntersectionY[0], saveIntersectionX[1], saveIntersectionY[1]);
  line(-saveIntersectionX[0], saveIntersectionY[0], -saveIntersectionX[1], saveIntersectionY[1]);
  line(-saveIntersectionX[0], -saveIntersectionY[0], -saveIntersectionX[1], -saveIntersectionY[1]);
  line(saveIntersectionX[0], -saveIntersectionY[0], saveIntersectionX[1], -saveIntersectionY[1]);
...
void step10() {
  ...
  myLineline3 = new Lineline(0, intersectionX, intersectionY, - topRightX[1], topRightY[1], outerX[3], outerY[3], outerX[7], outerY[7], false, false, 'b', 3);
  myLineline3.displayIntersection();

  println("IntersectionX[0] = " + saveIntersectionX[0]);
  println("IntersectionY[0] = " + saveIntersectionY[0]);

  line(intersectionX, intersectionY, - topRightX[1], topRightY[1]); // top left
...
void rectangle() {
  ...
  stroke(0, 0, 0);
  strokeWeight(1);
  line( outerX[7], outerY[7], outerX[13], outerY[13]);  // Left line of rect
  stroke(0);
...
void step8() {
  ...
  // todo find interscetion with top of rectangle
  // this becomes the end point of the guideline
  // look up starting point of 2nd blue line for starting point of new guideline

  println("About to call myLineline1");
  //  myLineline1 = new Lineline(1, 100, 100, 500, 500, 500, 100, 100, 500, true, true, 'm', 5);
  myLineline1 = new Lineline(1, -intersectionX, -middleScale -( middleScale+intersectionY), intersectionX, intersectionY, outerX[7], outerY[7], outerX[13], outerY[13], false, false, 'm', 2);
  myLineline1.displayIntersection();
  //println("IntersectionX[1] = " + saveIntersectionX[1]);
  //println("IntersectionY[1] = " + saveIntersectionY[1]);
...
class Lineline {
  int index;  // hold the index numember for the intersection use to store points in array
  float x1;
  float y1;
  float x2;
  float y2;
